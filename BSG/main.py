...
distance = 'cosine'
n_outputs = 300
upload_file_closest_articles = 'gs://bsg-image-recommendation/ALBEF/ALBEF_closest_articles_per_product.csv'
upload_file_embeddings = 'gs://bsg-image-recommendation/ALBEF/ALBEF_embeddings_of_products.csv'

# Getting products without description or image
data.fillna(value=np.nan)
data_without_description_or_image = data.loc[(data['description'].isnull()) | (data['images_link'].isnull())]
